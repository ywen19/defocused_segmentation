...
# -*- coding: utf-8 -*-
"""
Refiner training with enhanced curriculum:
...
def save_visualization(rgb, init_m, gt,
    ...
    # 前11张图：RGB, Init, GT, Refined, Guided, Unguided, Gate, ErrMap, ContErr, ShapeErr, AbsErr
    imgs = [to_np(t) for t in (r,i,gt0,rf,gd,ug,gtmp,sm0)]
    cont = np.clip(np.abs(to_np(rf)-to_np(gt0))/0.05,0,1)
    shp  = np.abs((to_np(rf)>0.5).astype(float)-to_np(gt0))
    absd = np.clip(np.abs(to_np(rf)-to_np(gt0)),0,1)
    imgs += [cont, shp, absd]

    titles = ["RGB","Init","GT","Refined","Guided","Unguided","Gate","ErrMap",
              "ContErr","ShapeErr","AbsErr","Gate Curve"]
    cmaps  = [None,"gray","gray","gray","gray","gray","viridis","hot",
              "jet","hot","hot", None]
    ...
    # 第12图：Gate μ 曲线（仅显示最近 max_history 点）
    ax = axs.flatten()[11]
    hist = gate_history[-max_history:]
    ax.plot(hist, '.-', linewidth=1, markersize=2)
    ax.set_title("Gate μ (last %d steps)"%len(hist))
    ax.set_xlabel("Step")
    ax.set_ylabel("μ")
    ax.set_ylim(0,1)
    ax.grid(True)
...
def train_with_error_map(model, loader, opt, scaler, device,
    ...
    for i, batch in enumerate(loader,1):
        ...
        loss = loss_ref
        if phase>=2:
            loss += 3.0 * F.l1_loss(gate_pix*(ug-gd), gt-gd) / accum_steps
        if phase>=3:
            if sub3==2:
                ...
                loss += 2.0*loss_gate + 0.05*loss_ent + 0.05*loss_sparse
            loss += 0.5*F.l1_loss(gd,gt)/accum_steps
            loss += 0.5*F.l1_loss(ug*get_soft_edges(gt),
                                 gt*get_soft_edges(gt)) / accum_steps
            with torch.no_grad():
                ll_gt,_,_ = custom_dwt(gt)
            _, ll0,_,_ = model.encoder(rgb, torch.zeros_like(mg), sm)
            true_ll    = ll_gt - ll0
            gate_ll    = F.interpolate(gate, size=true_ll.shape[-2:],
        ...
        if i % accum_steps == 0:
            print(f"Ep{epoch:02d} St{i:04d} "
                  f"Ltot={loss.item():.4f} Lref={loss_ref.item():.4f} "
                  f"Gateμ={gate.mean():.4f} σ={gate.std():.4f}")
        if i % print_int == 0:
            save_visualization(rgb, init_m, gt, rf, gd, ug, gate, sm,
