...
def get_soft_edges(mask):
    ...
    edge = torch.abs(F.conv2d(mask, lap, padding=1))
    edge = edge / (edge.max() + 1e-6)  # Normalize to [0,1]
    return edge

def soft_boundary_loss(pred, gt, sigma=1.0):
    pred_blur = gaussian_blur(pred, kernel_size=[5, 5], sigma=sigma)
    gt_blur = gaussian_blur(gt, kernel_size=[5, 5], sigma=sigma)
    return F.l1_loss(pred_blur, gt_blur)
...
def train_with_error_map(model, loader, optimizer, scaler, device, epoch,
    ...
    for i, batch in enumerate(loader):
        ...
        with autocast():
            ...
            edge_pred = get_soft_edges(m_pred)
            edge_gt = get_soft_edges(gt)
            loss_shape = F.mse_loss(edge_pred, edge_gt)

            loss_soft_boundary = soft_boundary_loss(m_pred, gt, sigma=1.0)
        ...
        if epoch >= 5 and teacher_model is not None:
            ...
            inverse_mask = 1.0 - structure_mask
