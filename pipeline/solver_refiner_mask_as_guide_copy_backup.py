...
def compute_dwt_loss(rf, gt, model, wavelet_list, lam_hf=1.0):
    ...
    print("RF level coeffs:", coeffs_rf[-1])
    print("  ll_rf:", type(ll_rf), ll_rf.shape)
    print("  hf_rf_list:", [type(x) for x in hf_rf_list])
...
def train_one_epoch(model, loader, opt, scaler, device,
    ...
    for i, (rgb, init_m, gt) in enumerate(loader, 1):
        rgb, init_m, gt = rgb.to(device), init_m.to(device), gt.to(device)
        # Prepare inputs
        ...
        # Compute losses
        loss = compute_phase1_loss(rf, gt, wave1, accum_steps)
        if epoch>3:
        ...
        if i%print_int==0:
            print(f"Train Epoch {epoch} [{i}/{len(loader)}] Loss: {loss.item():.4f}")
...
def val_one_epoch(model, val_loader, device, vis_dir, epoch, print_int):
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader, 1):
            ...
            # 损失计算 Phase 1
            loss = compute_phase1_loss(prediction, gt, wave1, accum_steps=1)
            # Phase 2
            if epoch > 3:
    ...
    print(f"Validate Epoch {epoch} Avg Loss: {avg_loss:.4f}")
...
if __name__=='__main__':
    ...
    for epoch in range(1, cfg['num_epochs']+1):
        ...
        val_one_epoch(model, val_loader, device, 'vis_val', epoch, cfg['print_interval'])
